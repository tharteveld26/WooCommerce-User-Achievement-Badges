...
if (!function_exists('wc_get_orders')) {
    add_action('admin_notices', function() {
        echo '<div class="notice notice-error"><p>WooCommerce User Achievement Badges requires WooCommerce to be active.</p></div>';
    });
    return;
}
...
jQuery(function($){
    if (typeof $.fn.select2 !== 'undefined') {
        window.requestAnimationFrame(() => {
            ...
            // Ensure fields show/hide on load
            updateFields();
        });
    }

    // Unlock type toggle
    const $typeField = $('#tbc_badge_type');
    ...
    $('.tbc-upload-badge-icon').on('click', function(e){
        e.preventDefault();
        const $input = $('#tbc_badge_icon');
        const $preview = $('.tbc-badge-media-preview');
        const frame = wp.media({ title: 'Select Badge Icon', button: { text: 'Use this icon' }, multiple: false });
